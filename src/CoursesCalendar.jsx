...
// TODO add full day event on first row
const makeEventsForHour = (year, month, dayNumber, hour, events) =>
  events.filter((event) => {
    // console.log({event, hour}, moment(event.startHour, "hh:mm:ss").hour())
        ...
        <Droppable droppableId={`${dayIndex}`} key={`key-${dayIndex}`}>
          {(provided) => (
            ...
            >
              ...
              {dayOfTheMonth.events
                // TODO remove implementation from logic
                // .filter((event) =>
                //   dayOfTheMonth.hour === 0
                //     ? !event.startHour
                //     : moment(`${event.startDate}T${event.startHour}`).format(
                //         "H"
                //       ) === `${dayOfTheMonth.hour - 1}`
                // )
